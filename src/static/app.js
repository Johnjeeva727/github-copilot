  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.map(p => `<li data-participant="${p}" data-activity="${name}"><span>${p}</span><button class="delete-btn" title="Remove participant" aria-label="Remove ${p}">×</button></li>`).join('');
        ...
        // Add delete button event listeners for this activity's participants
        activityCard.querySelectorAll(".delete-btn").forEach(btn => {
          btn.addEventListener("click", async (e) => {
            ...
            try {
              ...
              if (response.ok) {
                // Remove the list item with a smooth transition
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        // Refresh activities to update participant list and counts
        setTimeout(() => {
          fetchActivities();
        }, 500);
      } else {
